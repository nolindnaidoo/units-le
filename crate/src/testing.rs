...
//! only ever used by `walk`, `scan` and the MCP scan tool.
//!
...
//! spellings of the same directory — and every comparison between them
...
impl TempTree {
    pub(crate) fn new(name: &str) -> Self {
        ...
        let root =
            std::env::temp_dir().join(format!("units-le-{name}-{}-{unique}", std::process::id()));
